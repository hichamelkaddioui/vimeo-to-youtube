 ...
 * @return {Promise.<Object>} The authenticated oauth2Client
 */
const authenticatedOauth2Client = config => {
  ...
  return getLocalTokenOrAsk({ tokenPath: config.tokenPath, oauth2Client })
    .then(token => {
...
module.exports = authenticatedOauth2Client(youtubeConfig)
