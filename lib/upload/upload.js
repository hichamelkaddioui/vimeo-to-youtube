    ...
    task: () => new Observable(observer =>
      databaseUtil.markVideoAsUploading(video)
        ...
        .catch(err => databaseUtil.markVideoAsUploadFailed(video).then(() => {
          observer.error(new Error(`An error occured while uploading the video '${video.name}': ${err.message}`))
        }))
...
const worker = ({ limit, client }) => databaseUtil.findVideosToUpload(limit)
  .then(videos => {
    if (videos.length === 0) return []

    logger.debug(`Starting ${videos.length} uploads`)

    return client().then(youtube => new Listr(videos.map(video => uploadVideo({ video, youtube })), { concurrent: true, exitOnError: false }).run())
      .catch(err => logger.warn(`${err.message}`))
      .then(() => videos)
  })
  .then(res => res.length === 0 ? null : worker({ limit, client }))

module.exports = worker({ limit: uploadConfig.limit, client })
